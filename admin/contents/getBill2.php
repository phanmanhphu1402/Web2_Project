...
<body>
    ...
    $p = $_GET['p'];
    if($p==""&&$q==""){
        $sql = "select * from bill limit 0,8";
    }else{
        if($p!=""&&$q==""){
            $sql = "select * from bill where  Order_date <= '$p'";
        } else if($p==""&&$q!=""){
            $sql = "select * from bill where Order_date >= '$q'";
        } else{
            $sql = "select * from bill where Order_date >= '$q' AND Order_date <= '$p'";
        }
    }
