...
<body>
	<?php
	  ...
	  $sql = 'select * from product limit '.$index.',10';
	  $ListProduct = executeResult($sql);
        foreach($ListProduct as $Product){
            ...
            echo '<button type="button" class="btn btn-warning" onClick="lockUser('.$Product['ProductID'].')">Khóa</button>';
            
            echo '<button type="button" class="btn btn-danger" onClick="deleteUser('.$Product['ProductID'].');">Xóa</button>';
            echo '</td>';
            echo '</tr>';
            echo '';
        }
